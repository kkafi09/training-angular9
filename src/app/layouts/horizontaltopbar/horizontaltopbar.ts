...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
  ...
  // checkAkses(hakAkses) {
  //     return this.landaService.checkAkses(hakAkses, 'view');
  // }
  // checkAkses(hakAkses) {
  //   return this.landaService.checkAkses(hakAkses);
  // }

  modalPengumuman(dataModal, val) {
    this.modalService.open(dataModal, { size: 'lg', scrollable: true });
    this.dataModalPengumuman = {
      judul: val.judul,
      isi: val.isinya,
      tanggal: val.tanggal,
      penulis: val.penulis,
  ...
  savepasswords() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.landaService.DataPost('/pengguna/save1', data).subscribe((res: any) => {
